...
def main(user, passwd, step):
    ...
    #print(response)
    result = f"{user[:1]}7671{user[-1:]}: [{now}] 修改步数（{step}）"+ response['message']
    print(result)
    qqtalk = 'https://qmsg.zendee.cn/send/你申请到的Key?msg=' + "修改步数：" + step + "  " + response[
        'message'] + '&qq=你的qq号'
    requests.get(qqtalk)
    return result
...
def main_handler(event, context):
    ...
    user = "13145678982#13189339575"
    # 登录密码（用#隔开，例如123456#123456#123456）
    passwd = "gtv13189339575#gtv13145678982"
    # 要修改的步数，直接输入想要修改的步数值，留空为随机步数30000至99999之间
    step = ""
